...
type MatchResultsProps = {
  ...
  searchInput: MatchSkillRequestInput | null;
};

export function MatchResults({ isLoading, error, results, searchInput }: MatchResultsProps) {
  ...
  const handleRequestSwap = (instructorName: string) => {
    ...
    const newRequest: SkillRequest = {
      ...
      user: {
        name: instructorName,
        avatar: `https://placehold.co/100x100/A9A9A9/FFFFFF.png`,
        location: "Online",
      },
      skillOffered: searchInput.skillsOffered.join(', '),
      skillRequested: searchInput.skillRequested,
      status: 'pending',
      type: 'outgoing',
    };

    addRequest(newRequest);
    setSentRequests(prev => [...prev, instructorName]);
    ...
    <div className="mt-12">
      ...
      <div className="grid gap-6 md:grid-cols-1">
        {results.matches.map((match, index) => {
          const isSent = sentRequests.includes(match.instructorName);
          return (
            <Card key={index} className="flex flex-col transform hover:-translate-y-1 transition-transform duration-300 shadow-md hover:shadow-xl">
              ...
              <CardFooter>
                <Button 
                  className="ml-auto" 
                  onClick={() => handleRequestSwap(match.instructorName)}
                  disabled={isSent}
                >
                  {isSent ? (
                    <>
